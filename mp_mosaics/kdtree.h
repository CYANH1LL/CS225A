...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     KDTreeNode* helperKDTree(vector<Point<Dim>>& points_, int dimension, unsigned left, unsigned right);

     unsigned partition(vector<Point<Dim>>& newPoints, int dimension, unsigned left, unsigned right, unsigned pivotIndex);

     Point<Dim>& select(vector<Point<Dim>>& newPoints, int dimension, unsigned left, unsigned right, unsigned k);

     Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode* subroot, int dimension) const;

     double getDist(const Point<Dim>& query, const Point<Dim>& currentBest) const;
