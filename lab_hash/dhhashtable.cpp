...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    size_t hashIndex = hash(key, size);
    size_t interval = secondary_hash(key, size);


    while (table[hashIndex] != NULL) {
        hashIndex = (hashIndex + interval) % size;
    }

    table[hashIndex] = new pair<K, V> (key, value);;
...
void DHHashTable<K, V>::remove(K const& key)
{
    ...
    size_t hashIndex = hash(key, size);
    size_t interval = secondary_hash(key, size);
    while (table[hashIndex] != NULL && table[hashIndex]->first != key) {
        hashIndex = (hashIndex + interval) % size;
    }
