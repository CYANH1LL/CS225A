                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     } else if (mergeQueue.size() == 0 || singleQueue.front()->freq.getFrequency() < mergeQueue.front()->freq.getFrequency()) {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for(unsigned i = 0; i < frequencies.size(); i++) {
       TreeNode * insert = new TreeNode(frequencies[i]);
       singleQueue.push(insert);
     }
     ...
     while (singleQueue.size() + mergeQueue.size() > 1) {
       ...
       TreeNode * internal = new TreeNode(templeft->freq.getFrequency() + tempright->freq.getFrequency());
       internal->left = templeft;
       internal->right = tempright;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(bfile.getNextBit()) {
       Frequency * temp = new Frequency(bfile.getNextByte(), 0);
       output = new TreeNode(*temp);
       delete temp;
     } else  {
       Frequency * temp = new Frequency(0);
       output = new TreeNode(*temp);
